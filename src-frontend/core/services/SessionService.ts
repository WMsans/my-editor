...
export class SessionService {
    private p2p: P2PService; private auth: AuthService; private fs: FileSystemService; private collab: CollabService;

    // Heartbeat Config 
    private heartbeatInterval: any = null; private heartbeatTimeout: any = null; private readonly HEARTBEAT_RATE = 1000; // Send every 1s 
    private readonly HEARTBEAT_TIMEOUT = 2500; // Die if no signal for 2.5s

    // Format: ${hostId}:${rootPath} 
    private activeSessionKey: string | null = null;

    constructor(p2p: P2PService, auth: AuthService, fs: FileSystemService, collab: CollabService) { this.p2p = p2p; this.auth = auth; this.fs = fs; this.collab = collab; this.setupListeners(); }

    private setupListeners() {
        this.p2p.on('peer-connected', () => { const store = useSessionStore.getState(); store.setConnectedPeers(store.connectedPeers + 1); });
    ...
    private setHostRole(isHost: boolean) {
        useSessionStore.getState().setIsHost(isHost); this.collab.setRole(isHost);
    ...
    private startHeartbeatEmitter() { if (this.heartbeatInterval) clearInterval(this.heartbeatInterval); this.heartbeatInterval = setInterval(() => { this.p2p.broadcastUpdate('.heartbeat', []); }, this.HEARTBEAT_RATE); }

    private stopHeartbeatEmitter() { if (this.heartbeatInterval) clearInterval(this.heartbeatInterval); this.heartbeatInterval = null; }

    private startHeartbeatListener() { this.resetHeartbeatTimeout(); }

    private stopHeartbeatListener() { if (this.heartbeatTimeout) clearTimeout(this.heartbeatTimeout); this.heartbeatTimeout = null; }

    private resetHeartbeatTimeout() { if (this.heartbeatTimeout) clearTimeout(this.heartbeatTimeout); this.heartbeatTimeout = setTimeout(() => { this.handleHostLoss("heartbeat-timeout"); }, this.HEARTBEAT_TIMEOUT); }
    ...
    private handleHostLoss(id: string) {
        ...
        // [FIX] Reset active session key if host is lost
        if (this.activeSessionKey && this.activeSessionKey.startsWith(id)) {
    ...
    private async attemptTakeover() {
        const store = useSessionStore.getState(); const rootPath = useProjectStore.getState().rootPath; if (!rootPath) { store.setStatus('error', "Host disconnected. No project to takeover."); this.setHostRole(true); return; }
    ...
    public async negotiateHost(rootPath: string) {
        const store = useSessionStore.getState(); store.setStatus('negotiating');
        ...
        if (meta.hostId && meta.hostId !== myPeerId) {
            ...
            if (store.deadHostId !== meta.hostId) {
                ...
                // [FIX] Prevent duplicate join requests for the same session
                const sessionKey = `${meta.hostId}:${rootPath}`;
                if (this.activeSessionKey === sessionKey) {
                    console.log(`[SessionService] Already connected/negotiating with ${meta.hostId} for this project. Skipping duplicate join.`);
                    return;
                }
                ...
                this.activeSessionKey = sessionKey; // Mark session as active/negotiating
                ...
                try {
                    ...
                    this.activeSessionKey = null; // Reset on failure
                    store.setStatus('error', "Join request failed");
                }
                return;
            }
        }
        ...
        // Claim Host
        this.activeSessionKey = null; // Ensure we clear if we claim host
        await this.claimHost(rootPath, metaPath);

    }

    private async claimHost(rootPath: string, metaPath: string) {
        const store = useSessionStore.getState(); const myPeerId = this.p2p.getPeerId(); const myAddrs = this.p2p.getAddresses(); const key = this.auth.getKey();
