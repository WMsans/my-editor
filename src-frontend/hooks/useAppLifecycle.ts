import { useState, useEffect } from "react"; import { getCurrentWindow } from "@tauri-apps/api/window"; import { invoke } from "@tauri-apps/api/core"; import { pluginLoader } from "../mod-engine/PluginLoader";

interface UseAppLifecycleProps { rootPathRef: React.MutableRefObject<string>; sshKeyPathRef: React.MutableRefObject<string>; isHostRef: React.MutableRefObject<boolean>; setWarningMsg: (msg: string | null) => void; connectedPeersRef: React.MutableRefObject<number>; }

export function useAppLifecycle({ rootPathRef, sshKeyPathRef, isHostRef, setWarningMsg, connectedPeersRef }: UseAppLifecycleProps) { const [pendingQuit, setPendingQuit] = useState(false); const [isPushing, setIsPushing] = useState(false);

useEffect(() => { const win = getCurrentWindow(); const unlisten = win.onCloseRequested(async (event) => { const currentRoot = rootPathRef.current; const currentSsh = sshKeyPathRef.current;
         ...
         if (connectedPeersRef.current === 0) {
             ...
             try {
                 ...
                 const meta = { 
                    ...
                    requiredPlugins: pluginLoader.getEnabledUniversalPlugins() // [FIX] Persist universal plugins
                 };
                 ...
                 console.log("Wiped host ID (Session ended alone), preserved security settings and plugins.");
             } catch (e) {
